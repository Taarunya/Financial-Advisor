...
def init_purchase_table():
    ...
    CREATE TABLE IF NOT EXISTS purchases (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER NOT NULL,
        inr_paid REAL NOT NULL,
        virtual_added REAL NOT NULL,
        timestamp TEXT DEFAULT CURRENT_TIMESTAMP
    );
    """)
...
def add_virtual_cash(user_id: int, inr_paid: float, virtual_amount: float):
    init_purchase_table()
    ensure_wallet(user_id)
    ...
    cur.execute(
        ...
        (virtual_amount, user_id)
    )

    # Record purchase
    cur.execute("""
        INSERT INTO purchases (user_id, inr_paid, virtual_added)
        VALUES (?, ?, ?)
    """, (user_id, inr_paid, virtual_amount))
